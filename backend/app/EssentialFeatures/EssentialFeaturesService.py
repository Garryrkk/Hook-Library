...
def toggle_like_service(post_id, user):
    ...
    post = db.session.query(EssentialPost).filter(EssentialPost.id == post_id).first()
    if not post:
        raise HTTPException(status_code=404, detail="Post not found")

    existing_like = db.session.query(EssentialPostLike).filter_by(
...
def toggle_save_service(post_id, user):
    ...
    existing_save = db.session.query(EssentialPostSave).filter_by(
        user_id=user.id, post_id=post.id
    ).first()
...
def record_hook_view_service(hook_id):
    ...
    hook = db.session.query(EssentialHook).filter(EssentialHook.id == hook_id).first()
    if not hook:
...
def fetch_filtered_hooks_service(
    ...
    query = db.session.query(EssentialHook)
...
def reset_filters_service():
    ...
    hooks = db.session.query(EssentialHook).order_by(EssentialHook.id.desc()).all()
    return [h.to_dict() for h in hooks]
